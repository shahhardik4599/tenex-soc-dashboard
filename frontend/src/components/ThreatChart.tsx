...
import { MoveHorizontal } from 'lucide-react'; // <--- NEW IMPORT
...
export default function ThreatChart({ logs }: { logs: any[] }) {
    ...
    return (
        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-lg">
            ...
            {/* --- NEW INSTRUCTIONAL HELPER TEXT --- */}
            <div className="flex items-center justify-center mt-3 text-xs text-slate-400 font-medium">
                <MoveHorizontal className="w-4 h-4 mr-2 text-slate-500" />
                <span>Drag the highlighted center of the slider to pan, or drag the edges to zoom.</span>
            </div>
        </div>
    );
}